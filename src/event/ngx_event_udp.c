...
{
#define NGX_RECV_MSG_NUM 64
#define NGX_RECV_BUF_SIZE 4096
    ...
    ngx_event_t       *rev, *wev;
    struct iovec       iov[NGX_RECV_MSG_NUM];
    struct mmsghdr      msg[NGX_RECV_MSG_NUM];
    ngx_sockaddr_t     sa[NGX_RECV_MSG_NUM], lsa;
    struct sockaddr   *sockaddr, *local_sockaddr;
    ngx_listening_t   *ls;
    ngx_event_conf_t  *ecf;
    ngx_connection_t  *c, *lc;
    static u_char      buffer[NGX_RECV_MSG_NUM][NGX_RECV_BUF_SIZE];

#if (NGX_HAVE_MSGHDR_MSG_CONTROL)

#if (NGX_HAVE_IP_RECVDSTADDR)
    u_char             msg_control[NGX_RECV_MSG_NUM][CMSG_SPACE(sizeof(struct in_addr))];
#elif (NGX_HAVE_IP_PKTINFO)
    u_char             msg_control[NGX_RECV_MSG_NUM][CMSG_SPACE(sizeof(struct in_pktinfo))];
#endif

#if (NGX_HAVE_INET6 && NGX_HAVE_IPV6_RECVPKTINFO)
    u_char             msg_control6[NGX_RECV_MSG_NUM][CMSG_SPACE(sizeof(struct in6_pktinfo))];
#endif
    ...
    do {
        ...
        for (i = 0; i < NGX_RECV_MSG_NUM; i++) {
            iov[i].iov_base = (void *) buffer[i];
            iov[i].iov_len = NGX_RECV_BUF_SIZE;

            msg[i].msg_hdr.msg_name = sa + i;
            msg[i].msg_hdr.msg_namelen = sizeof(ngx_sockaddr_t);
            ...
            if (ls->wildcard) {
                ...
                if (ls->sockaddr->sa_family == AF_INET) {
                    msg[i].msg_hdr.msg_control = msg_control + i;
                    msg[i].msg_hdr.msg_controllen = sizeof(msg_control);
                }
#endif

#if (NGX_HAVE_INET6 && NGX_HAVE_IPV6_RECVPKTINFO)
                if (ls->sockaddr->sa_family == AF_INET6) {
                    msg[i].msg_hdr.msg_control = msg_control6 + i;
                    msg[i].msg_hdr.msg_controllen = sizeof(msg_control6);
                }
#endif
            }
        ...
        n = recvmmsg(lc->fd, msg, NGX_RECV_MSG_NUM, 0, NULL);
        if (n == -1) {
            ...
            ngx_log_error(NGX_LOG_ALERT, ev->log, err, "recvmmsg() failed");
        ...
        for (i = 0 ; i < n; i++) {
#if (NGX_HAVE_MSGHDR_MSG_CONTROL)
            if (msg[i].msg_hdr.msg_flags & (MSG_TRUNC|MSG_CTRUNC)) {
                ngx_log_error(NGX_LOG_ALERT, ev->log, 0,
                              "recvmmsg() truncated data");
                continue;
            }
#endif
            len = msg[i].msg_len;
            sockaddr = msg[i].msg_hdr.msg_name;
            socklen = msg[i].msg_hdr.msg_namelen;
            ...
            if (socklen == 0) {
                ...
                ngx_memzero(&sa[i], sizeof(struct sockaddr));
                sa[i].sockaddr.sa_family = ls->sockaddr->sa_family;
            }
            ...
            if (ls->wildcard) {
                ...
                for (cmsg = CMSG_FIRSTHDR(&msg[i].msg_hdr);
                     cmsg != NULL;
            ...
            *log = ls->log;
            ...
            c->buffer = ngx_create_temp_buf(c->pool, len);
            if (c->buffer == NULL) {
                ngx_close_accepted_udp_connection(c);
                return;
            }
    
            c->buffer->last = ngx_cpymem(c->buffer->last, buffer[i], len);
            ...
            if (log->log_level & NGX_LOG_DEBUG_EVENT) {
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_EVENT, log, 0,
                               "*%uA recvmmsg: %V fd:%d n:%z",
                               c->number, &addr, c->fd, len);
            }
